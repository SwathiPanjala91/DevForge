...
export default function ProblemEditorPage({ params }: { params: { slug: string } }) {
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      ...
      setErrorMessage("Execution Engine Offline: Judge0 is not configured for this environment.");
    }, 1500);
  };
  ...
  const handleRun = () => {
    ...
    setTimeout(() => {
      setSubmitState("error");
      setErrorMessage("Execution Engine Offline: Sandbox compilation failed to start.");
    }, 1000);
  };
        ...
        <div className="flex-[1.5] flex flex-col gap-4 min-h-0">
          <GlassCard className="flex-1 flex flex-col !p-0 overflow-hidden">
            ...
            <div className="flex items-center justify-between px-4 py-2 bg-slate-900/80 border-b border-white/10">
              ...
              <Button variant="ghost" size="sm" className="h-8 text-xs text-white/50 hover:text-white" leftIcon={<Settings className="w-3.5 h-3.5"/>}>
                Editor Settings
              </Button>
            </div>
