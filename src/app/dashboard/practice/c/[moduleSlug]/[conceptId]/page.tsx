...
export default function ConceptViewerPage({ params }: { params: Promise<{ moduleSlug: string, conceptId: string }> | { moduleSlug: string, conceptId: string } }) {
  const resolvedParams = React.use(params as any) as { moduleSlug: string, conceptId: string };
    ...
    <div className="min-h-screen bg-[#0f111a] text-gray-300 p-8 font-sans">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <nav className="text-sm text-gray-500 mb-4 flex items-center space-x-2">
          ...
          <span>/</span>
          <Link href={`/dashboard/practice/c/${moduleSlug}`} className="hover:text-blue-400 transition-colors">C</Link>
          <span>/</span>
          <span className="text-gray-300">{conceptId}</span>
        </nav>

        {/* Title */}
        <header className="border-b border-gray-800 pb-6">
          <h1 className="text-4xl font-bold text-white mb-2 tracking-tight">Understanding {conceptId} in C</h1>
        ...
        <section className="space-y-4">
          ...
          <p className="leading-relaxed">
            This module introduces the core concepts of {conceptId}. Understanding this is crucial for writing efficient and robust C programs.
