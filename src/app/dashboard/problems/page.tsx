    ...
    try {
      ...
      if (user) {
        try {
          ...
          console.warn("Could not get Firebase token for problems fetch:", e);
        }
      }
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-white/10 bg-white/5">
                  <th className="px-6 py-5 text-xs font-bold text-white/50 uppercase tracking-wider w-16 text-center">
                    Status
                  </th>
                  ...
                  <th className="px-6 py-5 text-xs font-bold text-white/50 uppercase tracking-wider text-right">
                    Action
                  </th>
                </tr>
              </thead>
                  ...
                  filteredProblems.map((p, idx) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <Link
                          href={`/dashboard/problems/${p.slug}`}
                          className="font-semibold text-white/90 group-hover:text-cyan-400 transition-colors text-base block mb-1"
                        >
                          {p.title}
                        </Link>
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`px-3 py-1.5 rounded-full text-xs font-bold border tracking-wide inline-block ${getDifficultyColor(
                            p.difficulty
                          )}`}
                        >
                          {p.difficulty}
                      ...
                      <td className="px-6 py-4 text-right">
                        <Link href={`/dashboard/problems/${p.slug}`}>
                          <Button
                            variant="ghost"
                            size="sm"
                            rightIcon={<ArrowRight className="w-4 h-4" />}
                            className="opacity-0 group-hover:opacity-100 transition-opacity bg-white/5 hover:bg-cyan-500/20 hover:text-cyan-400"
                          >
                            Solve
                          </Button>
                        </Link>
